...
```csharp
// Десериализация SBC/XML из файла — поддерживает xsi:type и все игровые типы
MyObjectBuilder_Definitions definitions;
MyObjectBuilderSerializer.DeserializePB<MyObjectBuilder_Definitions>(filePath, out definitions);

// Обобщённая версия для любого типа
MyObjectBuilderSerializer.DeserializePB<T>(filePath, out T result);
```

> Используем только публичный `MyObjectBuilderSerializer` (`VRage.ObjectBuilders`).
> Он обрабатывает `xsi:type`, никакой `Private` namespace не нужен.
...
## 11. Спавн из локального чертежа (полный алгоритм, публичные API)

```csharp
// 1. Загрузить bp.sbc (публичный MyObjectBuilderSerializer)
MyObjectBuilder_Definitions definitions;
MyObjectBuilderSerializer.DeserializePB<MyObjectBuilder_Definitions>(bpFilePath, out definitions);

// 2. Извлечь ShipBlueprint
var shipBp = definitions.ShipBlueprints[0];

// 3. Извлечь гриды
var grids = shipBp.CubeGrids.OfType<MyObjectBuilder_CubeGrid>().ToList();
