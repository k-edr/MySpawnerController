...
if (-not (Test-Path $mainProject)) {
    Write-Error "Main project dir not found: $mainProject"
    exit 1
}
if (-not (Test-Path $apiProject)) {
    Write-Error "API project dir not found: $apiProject"
    exit 1
}
...
# --- 2. Build API project (SDK-style, netstandard2.0) ---
Write-Host "[2/6] Building MySpawnerController.Api..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "API RESTORE FAILED" -ForegroundColor Red
...
Write-Host "[3/6] Building MySpawnerController..." -ForegroundColor Yellow
...
# Main plugin DLL (with retry - game may still be releasing file handles)
...
for ($i = 0; $i -lt 5; $i++) {
    try {
        Copy-Item -Path $mainDll.FullName -Destination $mainDllPath -Force -ErrorAction Stop
        $copied = $true
        break
    }
    catch {
        Write-Host "  Retry $($i+1)/5..." -ForegroundColor DarkYellow
        Start-Sleep -Seconds 2
    }
}
if (-not $copied) {
    Write-Error "Cannot copy main DLL - file locked. Kill SpaceEngineers.exe manually."
    exit 1
}
...
# System.Text.Json and transitive dependencies
...
# --- 5. Update config.xml ---
Write-Host "[5/6] Updating PluginLoader config.xml..." -ForegroundColor Yellow
